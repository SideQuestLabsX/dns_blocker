 ...
 *
 * Accepts one domain per line, and hosts-file lines such as
 * "0.0.0.0 ads.example.com". A leading address is dropped, comments and blank
 * lines are skipped, and names are lowercased.
 */
...
/* Reads the file back through the daemon's own code. The generator sorts and
...
static void SelfCheck(const char *path)
{
    FILE *file = fopen(path, "rb");
    if(file == NULL)
        Fatal("cannot reopen the output");

    fseek(file, 0, SEEK_END);
    long size = ftell(file);
    fseek(file, 0, SEEK_SET);

    uint8_t *bytes = Alloc((size_t)size);
    if(fread(bytes, 1, (size_t)size, file) != (size_t)size)
        Fatal("short read on the output");
    ...
    if(!BlocklistParseHeader(&list, bytes, (size_t)size))
        Fatal("the file we just wrote has a bad header");
...
int main(int argc, char **argv)
{
    if(argc < 2)
    {
        fprintf(stderr, "usage: mkblocklist out.trie [list ...]\n");
        return 1;
    }
    ...
    uint8_t header[BLOCKLIST_HEADER_BYTES];
    memcpy(header, BLOCKLIST_MAGIC, 4);
    PutU32(header + 4, nodeBytes);
    PutU32(header + 8, out.poolUsed);
    PutU32(header + 12, root->offset);
    ...
    fwrite(out.nodes, 1, nodeBytes, file);
    fwrite(out.pool, 1, out.poolUsed, file);
    fclose(file);

    SelfCheck(argv[1]);

    fprintf(stderr, "mkblocklist: %zu names, %zu skipped, %u bytes, checked\n",
            accepted, skipped,
            (unsigned)(sizeof header + nodeBytes + out.poolUsed));
    return 0;
}
