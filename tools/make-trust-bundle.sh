...
# The cap the daemon enforces, so the tool refuses what the daemon would refuse
cap=$(awk '/define[ \t]+CFG_TLS_CA_MAX_BYTES/ {
               for(i = 1; i <= NF; i++)
...
case "${cap:-}" in
    ''|*[!0-9]*)
        printf 'make-trust-bundle: cannot read CFG_TLS_CA_MAX_BYTES from %s\n' \
            "$config" >&2
        exit 1
        ;;
esac

# The resolvers this build authenticates, taken from the same header the daemon
# compiles, so the bundle cannot drift from the configuration.
...
hosts=${HOSTS:-$(printf '%s %s' "$resolvers" "$syncHosts")}
